...
const Percobaan2 = ({ apiData, shooterid }: any) => {
    ...
    const updateCheckmarksBE = async (updatedCheckmarks: boolean[]): Promise<UpdateHasilResponse> => {
        try {
            ...
            console.log(checkmarks);
            const response = await api.put(
                `/scorer/shooter/${shooterid}/result/stage6/2/checkmarks`,
                {
                    checkmarks: checkmarks,
                }
            );
    ...
    const handleNextNo = async (currentNo: number) => {
        ...
        if (confirmed) {
            const endpoint = `/scorer/shooter/${shooterid}/result/stage6/2/next`;
...
const Stage6 = () => {
    ...
    const finishPercobaan1 = async () => {
        ...
        if (confirmed) {
            const endpoint = `/scorer/shooter/${shooterid}/result/stage6/2`;
            setIsLoading(true);
            try {
