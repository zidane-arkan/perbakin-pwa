    ...
    const handleUpdateValuesNew = (e: React.FormEvent): void => {
        ...
        try {
            ...
            for (let i = 0; i < data.length; i++) {
                const key = `no_${i + 1}`;
                const currentItem = data[i];
                const pasanganStartIdx = Math.floor(i / 3) * 3;
                ...
                const scores_c = [
                    data[pasanganStartIdx + 2].nilaiPerkenaanA,
                    data[pasanganStartIdx + 2].nilaiPerkenaanC,
                    data[pasanganStartIdx + 2].nilaiPerkenaanD,
                ];
            ...
            const fixDataTry1 = {
                ...
                "checkmarks": [updatedData.checkmarks[0], updatedData.checkmarks[3]],
                "no_1": updatedData.no_1,
                "no_2": updatedData.no_4,
            }
...
const Stage5 = () => {
    ...
    const [stage5Data, setstage5Data] = useState<Try1Data | Try2Data | null | any>();
    ...
    const handleUpdateValues = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const response = await api.put(
                `/super/exam/${examid}/scorer/${scorerid}/shooter/${shooterid}/result/stage5`,
                stagesData
            );
    ...
    const fetchTry1Data = async () => {
        try {
            const response = await api.get(`/super/exam/${examid}/scorer/${scorerid}/shooter/${shooterid}/result/stage5`);
            const apiData = response.data;
            const stage5Data = apiData.data.stage_5;
            setstage5Data(stage5Data);
    ...
    const finishPercobaan1 = async () => {
        ...
        if (confirmed) {
            const endpoint = `/super/exam/${examid}/scorer/${scorerid}/shooter/${shooterid}/result/stage5/2`;
            setIsLoading(true);
            try {
    ...
    return (
        <section className="flex flex-col gap-8">
            ...
            <Styles>
                {'try_1' in stage5Data ? (
                    <Percobaan1 apiData={stage5Data.try_1} setStageTry1={setStageTry1} shooterid={shooterid} />
                ) : null}
                {!try2Status ? (
                ...
                {try2Status && 'try_2' in stage5Data ? (
                    <Percobaan2 setStageTry2={setStageTry2} apiData={stage5Data.try_2} shooterid={shooterid} />
            ...
            <form onSubmit={handleUpdateValues}>
                ...
                >
                    {isSaving ? 'Menyimpan...' : 'Simpan Stage 5'}
                </button>
            </form>
        </section>
    );
